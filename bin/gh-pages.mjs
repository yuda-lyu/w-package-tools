...
for (let i = 0; i < argv.length; i++) {
    if (argv[i] === '-d' && argv[i + 1]) { DIST = argv[i + 1]; i++ }
}
...
function runQuiet(cmd, opts = {}) { return execSync(cmd, { encoding: 'utf8', stdio: 'pipe', ...opts }).trim() }
...
let remoteUrl
try { remoteUrl = runQuiet(`git config --get remote.${REMOTE}.url`) }
catch {
...
if (existsSync(tmpAbs)) rmSync(tmpAbs, { recursive: true, force: true })
...
if (branchExists) {
    try { run(`git rm -rf .`, { cwd: tmpAbs }) }
    catch { console.log('[gh-pages] (無檔案可清空)') }
}

for (let entry of readdirSync(distAbs)) {
    cpSync(join(distAbs, entry), join(tmpAbs, entry), { recursive: true })
}
writeFileSync(join(tmpAbs, '.nojekyll'), '')

run(`git add -A`, { cwd: tmpAbs })
let hasChange = true
try { execSync(`git diff --cached --quiet`, { cwd: tmpAbs, stdio: 'ignore' }); hasChange = false } catch {}
if (hasChange) {
